...
class GatewayRunner:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        _msg_user_cfg = self._resolve_user_cfg(source)
        ...
        if _msg_tier_cfg is not None and _msg_tier_cfg.time_restrictions is not None:
            ...
            if not _time_ok:
                return self._format_tier_message(
                    _msg_tier_cfg,
                    _time_reason,
                    source,
        ...
        if _quick_key in self._running_agents:
            ...
            if not _is_stop:
                ...
                if (
                    ...
                    _int_ok, _int_reason = self._is_within_time_window(_msg_tier_cfg)
                    if not _int_ok:
                        return self._format_tier_message(
                            _msg_tier_cfg,
                            _int_reason,
        ...
        try:
            ...
            try:
                ...
                if pending:
                    ...
                    self._pending_approvals[self._approval_key(session_key, source)] = (
                        pending
                    )
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> str:
        ...
        if _tier_cfg is not None and not _tier_cfg.allow_exec:
            return self._format_tier_message(
                ...
                "exec_denied",
                event.source,
                user_cfg=self._resolve_user_cfg(event.source),
            )

        source = event.source
        session_key = self._session_key_for_source(source)
        approval_key = self._approval_key(session_key, source)
    ...
    def _resolve_user_tier(self, source):
        """Return tier name for user. Fallback: user config -> default_tier -> most-restrictive.
        ...
        tier_name = user_cfg.tier if user_cfg and user_cfg.tier else pt.default_tier
        ...
        if tier_name not in pt.tiers:
            ...
            if tier_name not in pt.tiers:
                ...
                tier_name = self._most_restrictive_tier(pt)
        return tier_name

    def _resolve_user_cfg(self, source):
        """Return UserTierConfig for user or None if unconfigured."""
        pt = getattr(getattr(self, "config", None), "permission_tiers", None)
        if pt is None:
            return None
        return pt.users.get(source.user_id) or pt.users.get("*")

    @staticmethod
    def _most_restrictive_tier(pt) -> str:
        """Return the name of the most-restrictive tier in pt.tiers.

        "Most restrictive" = fewest allowed_toolsets (with "*" counting as
        infinity), and among ties, no exec and no admin preferred.  If pt.tiers
        is empty (shouldn't happen due to F-1 warning, but defensive), returns
        a sentinel name that _get_tier_config() will map to None (unrestricted
        only when tiers are entirely absent).
        """
        if not pt.tiers:
    ...
    def _get_tier_config(self, tier_name):
        ...
        if pt is None:
            return None
        cfg = pt.tiers.get(tier_name)
        if cfg is None and tier_name == "__restricted_fallback__":
            # F-9: Sentinel tier for completely misconfigured configs.
            # Return maximally restrictive definition.
            from gateway.config import TierDefinition
    ...
    def _is_within_time_window(self, tier):
        ...
        if start_minutes <= end_minutes:
            # When start == end (e.g. "08:00"-"08:00"), this branch fires
            # and the user is always blocked: now >= end_minutes is always true
            # at the boundary. This is intentional — an equal start/end means
            # "zero-length window" (always restricted).
    ...
    def _format_tier_message(self, tier, key, source, user_cfg=None):
        ...
        if user_cfg:
            ...
            _resolved_cfg = pt.users.get(source.user_id) or pt.users.get("*")
            if _resolved_cfg:
